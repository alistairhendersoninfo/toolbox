...
def write_function(out, parent_id, items, children):
    ...
    for cid in children.get(parent_id, []):
        ...
        if not data:
            continue
        if data['type'] == 'menu':
            next_func = 'menu_level_root' if parent_id == '' else 'menu_level_' + data['num'].replace('.', '_')
            out.write(f"        {cid}) {next_func} ;;\n")
        else:
            out.write(f"        {cid}) bash -c '{data['cmd']}' ;;\n")
            out.write(f"        {cid}_msg) dialog --msgbox 'Executed: {data['num']} {data['mn']}' 5 50 ;;\n")
    out.write("    esac\n")
    out.write("}\n\n")
...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, parent_id, menu_number, mn, type, command, order_num FROM menu ORDER BY parent_id, order_num"
    )
    rows = cur.fetchall()
    conn.close()
    ...
    with open(output, 'w') as out:
        ...
        # Generate functions for root and submenus
