#!/usr/bin/env bash
#MN Apply Green Matrix Theme
#MD Applies green matrix terminal style theme to dialog
...
border_color = (BLACK,GREEN,ON)
button_active_color = (BLACK,GREEN,ON)
button_inactive_color = (BLACK,WHITE,ON)
EOF2

ln -sf "$CUSTOM_DIALOGRC" "$HOME/.dialogrc"
echo "Green Matrix theme applied."

TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')
db_write "INSERT INTO installs (software_name, task_group, task_name, status, updated_at) VALUES ('ToolboxColours', 'Theme Change', 'Green Matrix Theme', 'completed', '$TIMESTAMP');"
